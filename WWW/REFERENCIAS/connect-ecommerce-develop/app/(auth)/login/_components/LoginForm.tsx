...
function formatWhatsapp(value: string): string {
  ...
  const ccLen = digits.startsWith("55") ? 2 : Math.min(3, Math.max(1, digits.length - 11));
  const cc = digits.slice(0, ccLen);
  const rest = digits.slice(ccLen);

  if (cc === "55") {
    if (rest.length <= 2) return `+${cc} ${rest}`;
    if (rest.length <= 6) return `+${cc} (${rest.slice(0, 2)}) ${rest.slice(2)}`;
...
const verifySchema = z.object({
  token: z
    ...
    .regex(/^[0-9A-Za-z_-]{4,64}$/, "Token inválido"),
});
