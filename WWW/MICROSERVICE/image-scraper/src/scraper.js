...
export class ImageScraper {
  ...
  async run() {
    ...
    const strictProfile = {
      name: "strict",
      requireWhiteBgHint: self.requireWhiteBgHint,
      minAspectRatio: self.minAspectRatio,
      maxAspectRatio: self.maxAspectRatio,
      useVisualFilter: self.useVisualFilter,
      visualMinWhiteRatio: self.visualMinWhiteRatio,
      visualWhiteThreshold: self.visualWhiteThreshold,
      visualMaxObjectCoverage: self.visualMaxObjectCoverage,
      visualMinObjectCoverage: self.visualMinObjectCoverage,
      visualMaxComponents: self.visualMaxComponents,
    };
    if (self.targetType === "categoria") {
      strictProfile.requireWhiteBgHint = false;
      strictProfile.maxAspectRatio = Math.max(strictProfile.maxAspectRatio, 2.2);
    } else if (self.targetType === "marca") {
      strictProfile.requireWhiteBgHint = false;
    ...
    } else if (self.targetType === "banner") {
      strictProfile.requireWhiteBgHint = false;
    ...
    const relaxedProfile = {
      name: "relaxed",
      requireWhiteBgHint: self.relaxedRequireWhiteBgHint,
      minAspectRatio: self.relaxedMinAspectRatio,
      maxAspectRatio: self.relaxedMaxAspectRatio,
      useVisualFilter: self.relaxedUseVisualFilter,
      visualMinWhiteRatio: self.relaxedVisualMinWhiteRatio,
      visualWhiteThreshold: self.relaxedVisualWhiteThreshold,
      visualMaxObjectCoverage: self.relaxedVisualMaxObjectCoverage,
      visualMinObjectCoverage: self.relaxedVisualMinObjectCoverage,
      visualMaxComponents: self.relaxedVisualMaxComponents,
    };
    if (self.targetType === "categoria") {
    ...
    const candidateProfiles =
      self.retryNotFound && self.enableRelaxedRetryProfile ? [strictProfile, relaxedProfile] : [strictProfile];
    if (self.retryNotFound && self.enableRelaxedRetryProfile) {
    ...
    const crawler = new PlaywrightCrawler({
      ...
      async requestHandler({ request, page, log }) {
        ...
        try {
          ...
          const searchQueries = buildSearchQueries(query, self.retryNotFound, self.targetType);
          const providers = [
          ...
          for (const queryText of searchQueries) {
            for (const provider of providers) {
              ...
              candidates = mergeCandidates(candidates, found, self.maxCollectedCandidates);
            }
            if (candidates.length >= self.maxCollectedCandidates) break;
          }
          ...
          for (const cand of candidates.slice(0, self.maxCandidateEvaluations)) {
            try {
              const candUrl = String(cand?.url || "");
              const hint = `${cand?.hint || ""} ${candUrl}`;
              if (!candUrl) continue;
              if (self.targetProfile.shouldBlockPack && hasPackHint(hint)) continue;
          ...
          if (!imageUrl || !buffer) {
            ...
            if (self.placeholderOnNotFound) {
              const prodIndex = updatedProdutos.findIndex((p) => p.id === id);
              if (prodIndex >= 0) {
                updatedProdutos[prodIndex][imageField] = self.placeholderImageUrl;
                changes++;
              }
            }
          ...
          const ext =
            ...
            ".jpg";
          const fileName = `${self.targetProfile.assetFolder}/${slug}-${shortHash}${ext}`;
    ...
    const requests = targetProdutos.map(p => {
      const itemName =
        ...
        p?.label ||
        p?.slug ||
        `${self.targetProfile.fallbackName} ${p?.id ?? ""}`.trim();
      const itemBrand =
        p?.brand ||
        p?.marca ||
        p?.name ||
        self.targetProfile.fallbackBrand;
      return ({
