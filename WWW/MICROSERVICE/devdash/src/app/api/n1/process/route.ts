...
import { getN1Root } from "@/lib/n1/root";
...
async function stateFilePath() {
  ...
  return path.join(dir, "n1-process.json");
}

async function startLockFilePath() {
  const dir = await ensureDevDashDir();
  return path.join(dir, "n1-process.start.lock");
}
...
async function readState(): Promise<N1ProcessState | null> {
...
async function writeState(state: N1ProcessState) {
  const filePath = await stateFilePath();
...
async function startN1() {
  const configured = isLocalBaseUrl(process.env.DEVDASH_N1_BASE_URL ?? "http://localhost:3000");
  if (!configured) {
  ...
  const dir = await ensureDevDashDir();
  // Cada start do N1 gera um novo arquivo em `.devdash/` e faz pipe de stdout/stderr do processo
  ...
  const logFilePath = path.join(dir, `n1-${Date.now()}.log`);
  ...
  try {
    ...
    // Captura logs do processo (stdout/stderr) no arquivo `.devdash/n1-<timestamp>.log`.
    ...
    if (!state.pid) {
      ...
      await clearState();
      return NextResponse.json({ ok: false, error: "spawn_error", details: outcome.error, cwd, logFilePath }, { status: 500 });
    }
...
export async function POST(req: Request) {
  ...
  if (action === "start") return await startN1();
  if (action === "stop") return await stopN1();
