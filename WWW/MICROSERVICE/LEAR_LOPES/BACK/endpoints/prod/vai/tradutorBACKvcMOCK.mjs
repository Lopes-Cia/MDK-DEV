...
function detectBadges(text) {
  ...
  const isBebida =
    ...
    t.includes("isotônico");
  return isBebida ? ["gelada"] : [];
}
...
async function main() {
  ...
  const inJ1 = resolveCliPath(getArgValue("--in-j1"), "..\\j1.json");
  const inCatList = resolveCliPath(getArgValue("--in-cat-list"), "..\\..\\..\\data\\CATEGORIA\\list.json");
  const inBrandList = getArgValue("--in-brand-list")
    ? resolveCliPath(getArgValue("--in-brand-list"), "brands.list.json")
    : "";

  const outDir = resolveCliPath(getArgValue("--out-dir"), "OUT");
  ...
  process.stdout.write(
    `${JSON.stringify(
      { ok: true, inputs: { inJ1, inCatList, inBrandList: inBrandList || null }, outputs: { outProdutos, outCategorias, outBrands }, counts: { produtos: produtos.length, categorias: categorias.length, brands: brands.length } },
      null,
      2
    )}\n`
  );
