...
async function main() {
  ...
  const envDir = path.resolve(__dirname, "..", "BACK");
  const env = await loadEnvFrom(envDir);
  const port = toPort(env.HTTP_SERVER_PORT, 3100);

  const tokenManager = new TokenManager({ envDir, env });
  const server = new HttpServer({ envDir, env, tokenManager, port, label: "BACK" });
  await server.start();

  process.stdout.write(`BACK: http://localhost:${port}\n`);
}
