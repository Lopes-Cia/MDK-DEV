...
- Base: `/api`
- Cliente: `apiClient(endpoint)` concatena `'/api' + endpoint` ([client.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/api/client.ts#L1-L55))

## Variáveis de ambiente (.env)

Leitura e normalização em [config.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/integration/config.ts#L1-L74).
...
|---|---|---|---|---|
| POST | `/api/auth/register` | `{ responsavel, cnpj, email, whatsapp }` | `{ success, data }` | [route.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/app/api/auth/register/route.ts#L8-L94) |
| POST | `/api/auth/send-token` | `{ email?, whatsapp? }` (um dos dois) | `{ success, data }` | [route.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/app/api/auth/send-token/route.ts#L8-L89) |
| POST | `/api/auth/verify-token` | `{ token }` | `{ success, data: { verification, operador } }` | [route.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/app/api/auth/verify-token/route.ts#L9-L141) |
| POST | `/api/auth/logout` | — | `{ success }` | [route.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/app/api/auth/logout/route.ts#L1-L25) |
| GET | `/api/auth/me` | — | `{ success, data: session }` ou `401` | [route.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/app/api/auth/me/route.ts#L1-L38) |

Clientes (consumo no front):
- [registerUser](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/api/auth.ts#L12-L19)
- [sendLoginToken](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/api/auth.ts#L21-L28)
- [verifyLoginToken](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/api/auth.ts#L30-L40)
- [logout](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/api/auth.ts#L42-L46)
- [getCurrentSession](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/api/auth.ts#L48-L52)

### Sessão (cookie)

- Cookie: `session`
- Formato: JSON em claro (httpOnly) com shape `Session`:
  - `{ userId: string; email: string; token: string; name?: string }` ([session.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/auth/session.ts#L1-L42))
- Escrita: no `POST /api/auth/verify-token`, após validar token e obter operador ([verify-token route](../../WWW/REFERENCIAS/connect-ecommerce-develop/app/api/auth/verify-token/route.ts#L114-L119))
- Proteção: middleware redireciona `/dashboard/*` para `/login` se não houver cookie `session` ([middleware.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/middleware.ts#L1-L18))
...
|---|---|---|---|---|
| POST | `/tokenService` | Body: `{ produto, ean, idIntegradora, codCli }` | `Content-Type: application/json` | Geração do token do integrador ([authService.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/integration/authService.ts#L79-L107)) |
| POST | `/tokenService` | Body: `{ refreshToken }` | `Content-Type: application/json` | Refresh do token ([authService.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/integration/authService.ts#L109-L140)) |
| POST | `/postAutenteicaAplicativo` | Body: `{ chaveAtivacao, responsavel, cnpj, email, whatsapp }` | `Content-Type: application/json`, `Authorization` | Cadastro ([register route](../../WWW/REFERENCIAS/connect-ecommerce-develop/app/api/auth/register/route.ts#L37-L60)) |
| POST | `/enviarToken` | Query: `email=...` ou `whatsapp=...` | `Authorization` | Envia token de login ([send-token route](../../WWW/REFERENCIAS/connect-ecommerce-develop/app/api/auth/send-token/route.ts#L33-L55)) |
| POST | `/verificarTokenSistema` | Query: `token=...` | `Authorization` | Valida token de login ([verify-token route](../../WWW/REFERENCIAS/connect-ecommerce-develop/app/api/auth/verify-token/route.ts#L56-L68)) |
| GET | `/getOperadorSistemaForId` | Query: `id=...` | `Authorization` | Carrega operador autenticado ([verify-token route](../../WWW/REFERENCIAS/connect-ecommerce-develop/app/api/auth/verify-token/route.ts#L85-L97)) |

Formato do header:
- `Authorization` recebe o token “cru” (se vier com prefixo `Bearer `, ele é removido) ([token.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/integration/token.ts#L1-L11))

Response esperada de `/tokenService` (mínimo validado pelo app):
- `{ hashToken: string; dtExpira: string; refreshToken?: string; ... }` ([integration.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/types/integration.ts#L1-L6))

## Produtos

| Método | Path | Query | Retorno | Implementação |
|---|---|---|---|---|
| GET | `/api/products` | `idIntegradora?: number` | `{ success, data, total }` | [route.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/app/api/products/route.ts#L1-L48) |
| GET | `/api/products/:codProd` | `idIntegradora?: number` | `{ success, data }` | [route.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/app/api/products/%5BcodProd%5D/route.ts#L1-L65) |

Clientes (consumo no front):
- [getProducts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/api/products.ts#L15-L28)
- [getProductById](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/api/products.ts#L30-L47)
...
|---|---|---|---|---|
| GET | `/Servidor/webservice/integration/getIntegradora` | `id=<idIntegradora>` | `Authorization` | Boot da integração (carrega config) ([authService.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/integration/authService.ts#L142-L165)) |
| GET | `/Servidor/webservice/integration/getListProdutoLoja` | `idIntegradora=<id>` | `Authorization` | Lista produtos ([productsService.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/integration/productsService.ts#L66-L74)) |
| GET | `/Servidor/webservice/integration/getProdutoLoja` | `idIntegradora=<id>&codProd=<codProd>` | `Authorization` | Detalhe de produto ([productsService.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/integration/productsService.ts#L76-L88)) |

Payloads esperados (tolerantes):
- Lista pode vir como array direto ou dentro de `data|produtos|products|lista|itens` ([productsService.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/integration/productsService.ts#L22-L42))
- Item pode vir como objeto direto ou dentro de `data|produto|product|item` ([productsService.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/integration/productsService.ts#L44-L64))

Shape do `Product` (campos principais):
- Tipagem em [product.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/types/product.ts#L13-L54) (ex.: `codProd`, `idIntegradora`, `descricaoEcomerce`, `preco`, `qtEstoque`, `imagem`, `imagens`, `categoria`, `departamento`, etc.)
...
Objetivo: permitir que o microservice [MOCK-END](../../WWW/MICROSERVICE/MOCK-END/server.mjs) simule **tanto** os endpoints internos do app (`/api/auth/*`, `/api/products*`) **quanto** os endpoints externos (`AUTH_BASE_URL` e `INTEGRATION_URL_API`), mantendo o contrato descrito acima.
...
- Chamada direta no browser: `GET /api/products` (query opcional `idIntegradora`). Não envia `Authorization` nem parâmetro `tenant` (ver [page.tsx](../../WWW/REFERENCIAS/connect-ecommerce-develop/app/(shop)/page.tsx#L19-L36) e [products.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/api/products.ts#L15-L28)).
- Token necessário é do integrador e acontece no server-to-server (dentro do handler de `/api/products`), que por sua vez chama o fluxo de integração (`/tokenService` e `/Servidor/webservice/integration/getListProdutoLoja`) usando `AUTH_BASE_URL`/`INTEGRATION_URL_API` do `.env` (ver [route.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/app/api/products/route.ts#L8-L34) e [productsService.ts](../../WWW/REFERENCIAS/connect-ecommerce-develop/lib/integration/productsService.ts#L66-L74)).
